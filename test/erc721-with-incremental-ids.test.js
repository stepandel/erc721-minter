...
const contractName = "ERC721WithIncrementalIds"
...
describe('ERC721WithIncrementalIds', function() {
  ...
  describe("Out of Order Mint", function() {
    ...
    it('mint tokenId #1 - success', async function() {
    ...
    it('mint tokenId #7 - fail', async function() {
      ...
      await expect(this.registry.redeem(this.token.account, this.token.tokenId, METADATA_URL, this.token.signature, { value: ethers.utils.parseEther(NFT_PRICE) }))
        .to.be.revertedWith('TokenId already minted or out of order');
    });
  })
